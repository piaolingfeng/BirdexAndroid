...
public class MsgOrderAdapter extends RecyclerView.Adapter<MsgOrderAdapter.MsgOrderHolder> {
    ...
    public MsgOrderAdapter(Context mContext, List<OrderListEntity.OrderListNum.Orders> list, String currentName) {
    ...
    public void onBindViewHolder(MsgOrderHolder holder, final int position) {
        ...
        holder.tv_create_time.setText(list.get(position).getCreated_time());
        holder.tv_order_num.setText(list.get(position).getOrder_oms_no());
        holder.tv_recevice_addr.setText(list.get(position).getReceiver_name() + "," + list.get(position).getReceiver_mobile());//缺了收件人地址
        holder.tv_error.setText(list.get(position).getVerify_fail_detail());
        ...
        holder.tv_function.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (currentName.contains(mContext.getString(R.string.msg_check_exception)))
                    ...
                    upLoadIDCard(mContext,list.get(position).getOrder_code(),"");//订单列表没有身份证id,只能穿order_code
            }
        });
        if (currentName.contains(mContext.getString(R.string.msg_repertory_exception))) {//库存异常显示
            ...
            MsgOrderProductAdapter adapter = new MsgOrderProductAdapter(mContext, list.get(position).getProducts());
